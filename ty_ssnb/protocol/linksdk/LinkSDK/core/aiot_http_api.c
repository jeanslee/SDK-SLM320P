...
static void _core_http_auth_recv_handler(void *handle, const aiot_http_recv_t *packet, void *userdata)
{
    ...
    switch (packet->type) {
        ...
        case AIOT_HTTPRECV_HEADER: {
			...
			else if ((strlen(packet->data.header.key) == strlen("Content-Range")) &&
				(memcmp(packet->data.header.key, "Content-Range", strlen(packet->data.header.key)) == 0)) {
				core_str2uint(packet->data.header.value, (uint8_t)strlen(packet->data.header.value), &response->actual_total_len);
			}
        }
        break;
    ...
    switch (packet->type) {
        ...
        case AIOT_HTTPRECV_HEADER: {
            if ((strlen(packet->data.header.key) == strlen("Content-Length")) &&
                (memcmp(packet->data.header.key, "Content-Length", strlen(packet->data.header.key)) == 0)) {
                core_str2uint(packet->data.header.value, (uint8_t)strlen(packet->data.header.value), &response->content_total_len);
                LK_LOG("%s %d content_total_len=%d\r\n",__FUNCTION__,__LINE__,response->content_total_len);
                
            }
			else if ((strlen(packet->data.header.key) == strlen("Content-Range")) &&
				(memcmp(packet->data.header.key, "Content-Range", strlen(packet->data.header.key)) == 0)) {
				core_str2uint(packet->data.header.value, (uint8_t)strlen(packet->data.header.value), &response->actual_total_len);
                LK_LOG("%s %d actual_total_len=%d\r\n",__FUNCTION__,__LINE__,response->actual_total_len);
			}
            if(http_handle->recv_handler)
        ...
        case AIOT_HTTPRECV_BODY: {
            ...
            LK_LOG("%s %d response->content_len=%d\r\n",__FUNCTION__,__LINE__,response->content_len);
            LK_LOG("%s %d packet->data.body.len=%d\r\n",__FUNCTION__,__LINE__,packet->data.body.len);
            uint8_t *content = http_handle->sysdep->core_sysdep_malloc(response->content_len + packet->data.body.len + 1,
                               CORE_HTTP_MODULE_NAME);
            if (content == NULL) {
                return;
...
static int32_t _core_http_send_post_with_range(core_http_handle_t *http_handle, char *path, char *content, uint32_t startRange, uint32_t endRange)
{
	int32_t res = STATE_SUCCESS;
	//char *content = NULL, *path = NULL;
	//char *path_fmt = "/auth?_v=%s&%s";
	...
	core_http_request_t request;
	char range_start_string[10] = { 0 };
	char range_end_string[10] = { 0 };
	uint8_t range_start_string_len = 0;
	uint8_t range_end_string_len = 0;
	char *header_string = NULL;
	core_uint2str(startRange, range_start_string, &range_start_string_len);
	core_uint2str(endRange, range_end_string, &range_end_string_len);

	char *prefix =  (http_handle->long_connection == 0) ? ("Content-Type: application/json\r\nConnection: close\r\nRange: bytes=") :
		("Content-Type: application/json\r\nRange: bytes=");
	...
	res = core_sprintf(http_handle->sysdep, &header_string, "%s%s%s%s\r\n", src, len, CORE_HTTP_MODULE_NAME);
	if (res != STATE_SUCCESS) {
		if (header_string) {
			http_handle->sysdep->core_sysdep_free(header_string);
		}
		return res;
	}
...
static int32_t _core_http_send_get_width_range(core_http_handle_t *http_handle, char *path, uint32_t startRange, uint32_t endRange)
{
	int32_t res = STATE_SUCCESS;
	//char *content = NULL, *path = NULL;
	//char *path_fmt = "/auth?_v=%s&%s";
...
static int32_t _core_http_send_post(core_http_handle_t *http_handle, char *path, char *content)
{
	int32_t res = STATE_SUCCESS;
	//char *content = NULL, *path = NULL;
	//char *path_fmt = "/auth?_v=%s&%s";
...
static int32_t _core_http_recv_post(core_http_handle_t *http_handle, core_http_response_t *response)
{
	int32_t res = STATE_SUCCESS;
	char *token = NULL;
    ...
    LK_LOG("%s %d ,response->code=%d\r\n",__FUNCTION__,__LINE__,response->code);
	if (response->code != 200 && response->code != 206 && response->code != 204) {
		return STATE_HTTP_AUTH_CODE_FAILED;
	}
	if (response->content == NULL || response->content_len != response->content_total_len) {
		if (response->code == 204)
		{
			const char *emptyjson = "{\"header\" : {\"functionExecutionStatus\" : {\"status\" : \"Executed-Success\"}}}";
			if (response->content != NULL)
			{
				http_handle->sysdep->core_sysdep_free(response->content);
			}
			response->content = http_handle->sysdep->core_sysdep_malloc(strlen(emptyjson) + 1, "204 header");
			memset(response->content, 0x00, strlen(emptyjson) + 1);
			response->content_len = strlen(emptyjson)+1;
			response->content_total_len = response->content_len;
			strcpy(response->content, emptyjson);
	//		printf("204 can be null");
		}
		else {
...
int32_t aiot_http_post_with_range(void *handle, char *path, char *content, char **response_content, uint32_t* totalLen, uint32_t startRange, uint32_t endRange)
{
	int32_t res = STATE_SUCCESS;
	core_http_response_t response;
	core_http_handle_t *http_handle = (core_http_handle_t *)handle;
	...
	/* send post request */
	res = _core_http_send_post_with_range(http_handle, path, content, startRange, endRange);
	if (res < STATE_SUCCESS) {
		_core_aiot_http_exec_dec(http_handle);
		return res;
	}
	...
	/* recv post response */
	res = _core_http_recv_post(http_handle, &response);
	if (response.content_total_len > 0) {
		//memcpy(response_content, response.content, response.content_total_len);
		*response_content = response.content;
		if (response.actual_total_len > 0) {
			*totalLen = response.actual_total_len;
		}
		else {
			*totalLen = response.content_total_len;
		}
	}
	//在post返回之后，外层自己释放
	//if (response.content != NULL) {
...
int32_t aiot_http_get(void *handle, char *path, uint8_t **response_content, uint32_t* responseLen, uint32_t* totalLen,uint32_t startRange, uint32_t endRange)
{
	int32_t res = STATE_SUCCESS;
	core_http_response_t response;
	core_http_handle_t *http_handle = (core_http_handle_t *)handle;
	...
	/* send post request */
	res = _core_http_send_get_width_range(http_handle, path, startRange, endRange);
	if (res < STATE_SUCCESS) {
		_core_aiot_http_exec_dec(http_handle);
		return res;
	}
	...
	if (response.content_total_len > 0) {
		//memcpy(response_content, response.content, response.content_total_len);
		*response_content = response.content;
		*responseLen = response.content_total_len;
		if (response.actual_total_len > 0) {
			*totalLen = response.actual_total_len;
		}
		else {
...
int32_t aiot_http_post(void *handle,char *path, char *content, char **response_content, uint32_t *totalLen)
{
	int32_t res = STATE_SUCCESS;
	core_http_response_t response;
	core_http_handle_t *http_handle = (core_http_handle_t *)handle;
	...
	/* send post request */
	res = _core_http_send_post(http_handle,path,content);
	if (res < STATE_SUCCESS) {
		_core_aiot_http_exec_dec(http_handle);
		return res;
	}
