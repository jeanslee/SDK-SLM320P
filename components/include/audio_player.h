...
/**
 * \brief aw8733 auido PA control
 *
 * When audio player is not started, or audio reader is not managed
 * by audio player, it may return NULL.
 *
 * \param enable  PA enable switch
 * \return
 *     -true - turn on audio PA
 *     -false -turn off audio PA
 */

bool aw8733_audio_pa_ctrl(bool enable,uint8_t edge_num);
OSI_EXTERN_C_END
#endif
