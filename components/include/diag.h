...
/**
 * @brief query the CPU sequence number and store in the memory indicated by the pointer
 * @param uidNum     pointer of type char, must allocate more than 16 bytes of memory,
 *                      bit[15]-bit[0] is CPU serial number, bit[16] is '/0'
 * @param len        the size of memory indicated by the pointer
 */
void MG_handleReadUIDReq(char *uidNum, int len);
