...
/*Meig API description*/
bool mg_drvLcdInit(void);
void mg_drvLcdClose(void);
void mg_drvLcdGetLcdInfo(lcdSpec_t *lcd_info_ptr);
bool mg_drvLcdBlockTransfer(const lcdFrameBuffer_t *dataBufferWin, const lcdDisplay_t *lcdRec);
bool mg_drvLcdBlockSetOverLay(const lcdFrameBuffer_t *dataBufferWin);
bool mg_drvLcdSetDisplayWindow(uint16_t left, uint16_t top, uint16_t right, uint16_t bottom);
bool mg_drvLcdSetImageLayerSize(uint32_t width, uint32_t height, uint32_t imageBufWidth);
void mg_drvLcdEnterSleep(bool is_sleep);
uint32_t drvLcdGetSensorID(void);

/**---------------------------------------------------------------------------*
 **                         Compiler Flag                                     *
 **---------------------------------------------------------------------------*/
void lcd_DVDD_on(bool on);
void lcd_IODVDD_on(bool on);
OSI_EXTERN_C_END
