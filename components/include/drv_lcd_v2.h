...
void MG_drvLcdSetBackLightEnable(void);

void MG_drvLcdSetBackLightDisable(void);
...
void MG_drvLcdBacklightBrightness(uint8_t level);
void display_logo(int8_t mode);
void drvLcdSetState(drvLcd_t *d,uint8_t state);
uint32_t drvLcdGetChipID(void);
void drvRotationLcdBuffer(uint8_t mode,uint16_t *in_buf,uint16_t *out_buf,unsigned width, unsigned height);
OSI_EXTERN_C_END
#endif
